...
interface IVerifier{
    function verifySignature(bytes memory data, bytes memory signature) external returns(bool);
    function getTreasuryAddress(bytes memory signature) external returns(address);
...
contract StrategyManager {
    ...
    function openPosition(uint premiumAmount, bytes memory strategyParameters, bytes memory strategyParametersSignature) public{
        /* TODO: Add era period validation */
        uint64 normalizedPremiumAmount = uint64(premiumAmount / WEI_PER_UNIT);

        require(signatureVerifier.verifySignature(strategyParameters, strategyParametersSignature),"strategy-manager/not-signed-parameters");

        address treasuryAddress = signatureVerifier.getTreasuryAddress(strategyParametersSignature);

        uint64 positionSize = IKnoxStrategy(strategies[treasuryAddress]).getPositionSize(normalizedPremiumAmount, strategyParameters);
        ITreasury treasury = ITreasury(treasuryAddress);
        int32 epochId = treasury.currentEpoch();

        treasury.trustedBorrow(uint256(positionSize)*WEI_PER_UNIT);

        require(treasury.baseToken().transferFrom(msg.sender, address(this), positionSize), "strategy-manager/premium-transfer-impossible");
        bytes memory callData = abi.encodeWithSignature("openPosition(bytes, bytes)",abi.encode(strategyParameters, normalizedPremiumAmount));
        (bool status,) = address(strategies[treasuryAddress]).delegatecall(callData);

        require(status,"strategy-manager/strategy-invocation-failed");

        openedPositions[msg.sender].push(PositionRecord(epochId, positionSize, address(treasury)));
        totalPositionSize[treasuryAddress][epochId] = totalPositionSize[treasuryAddress][epochId] + positionSize;
        
    }

    function settlePosition(address treasuryAddress) public{ /*called once per strategy/treausery for era by administrator*/
        ...
        (bool status, bytes memory retVal) = address(strategies[treasuryAddress]).delegatecall(callData);
    ...
    function withdrawPayout(uint256 index) public{/*called independly by any MM to get payaout associated with position*/
        ...
        uint64 payout = uint64( uint256(rec.positionSize)* uint256(totalPayout)/uint256(totalPositionSize));
