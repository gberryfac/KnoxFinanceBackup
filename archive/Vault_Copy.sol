...
contract Vault {
    ...
    function syncFarmer(address _farmer) private {
        ...
        if (currentEpoch != -1 && depositor.lastEpoch < currentEpoch) {
            if (depositor.waitingAmount < 0) {
                ...
                amountOwned -= withdrawalAmount;
                totalWaitingAmount -= int64(uint64(withdrawalAmount));
    ...
    function withdraw(uint256 amount) public {
        ...
        require(
            depositor.waitingAmount > 0 ||
                getAmountOwned(depositor.normalizedBalance) > 0,
            "vault/no-funds-deposited"
        );
        ...
        if (epochs[currentEpoch].totalRepayedAmount > 0) {
            ...
            if (amountInUnits > 0) {
                if (depositor.waitingAmount > amountInUnits) {
                    // instantly withdraw partial waiting deposit
                    require(
                        baseToken.transfer(
                            address(msg.sender),
                            uint256(int256(amountInUnits)) * WEI_PER_UNIT
                    ...
                    totalWaitingAmount -= amountInUnits;
                } else if (depositor.waitingAmount > 0) {
                    // instantly withdraw entire waiting deposit
                    require(
                        baseToken.transfer(
                            address(msg.sender),
                    ...
                    totalWaitingAmount -= depositor.waitingAmount;
                } else if (depositor.waitingAmount < 0) {
                    totalWaitingAmount -= amountInUnits;
                } else if (depositor.waitingAmount == 0) {
                    totalWaitingAmount += amountInUnits;
        ...
        depositor.waitingAmount -= amountInUnits;
        depositors[msg.sender] = depositor;
    }
