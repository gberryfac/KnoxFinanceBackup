...
import { FarmersTreasury } from "./../typechain-types/FarmersTreasury";
...
describe.only("Farmers Treasury", () => {
  ...
  before(async function () {
    ...
    let block = await hre.ethers.provider.getBlock(await hre.ethers.provider.getBlockNumber());
    ...
    depositedToken = await factoryERC20.deploy("TestToken", "TT", parseUnits('1', 'ether')) as TestERC20;
    await depositedToken.transfer(signer2.address, parseUnits('500', 'finney'));
    treasury = (await factoryFarmersTreasury.deploy(depositedToken.address, block.timestamp, signer2.address)) as FarmersTreasury;
  ...
  describe("withdraw", () => {
    ...
    describe("in the same epoch", () => {
      ...
      beforeEach(async () => {
        ...
        await depositedToken.approve(treasury.address, parseUnits('2000', 'gwei'));
        ...
        await treasury.connect(signer2).trustedBorrow(parseUnits('1000', 'gwei'));
      ...
      it("should immediately withdraw sum equal this epoch deposit", async () => {
        ...
        expect(balanceBefore.add(parseUnits('900', 'gwei')).toString()).to.be.equal(balanceAfter.toString());
      });
      ...
      it("should immediately withdraw sum smaller than this epoch deposit", async () => {
        ...
        await tx.wait();
        let balanceAfter = await depositedToken.balanceOf(signer.address);
        expect(balanceBefore.add(parseUnits('800', 'gwei')).toString()).to.be.equal(balanceAfter.toString());
      });
    ...
    describe("in next epoch", () => {
      ...
      beforeEach(async () => {
        initSnapshotId = await hre.ethers.provider.send("evm_snapshot", []);
        await depositedToken.approve(treasury.address, parseUnits('2000', 'gwei'));
        await depositedToken.connect(signer2).approve(treasury.address, parseUnits('5000', 'gwei'));
        await treasury.deposit(parseUnits('1000', 'gwei'));
        ...
        await treasury.deposit(parseUnits('900', 'gwei'));
        //console.log("DEP 900");
        await treasury.connect(signer2).trustedRepay(parseUnits('1200', 'gwei'));
        await treasury.connect(signer2).createNewEpoch();
  ...
  describe("deposit", () => {
    ...
    it("should succed if approval is set", async function () {
      await depositedToken.approve(treasury.address, parseUnits('1000', 'gwei'));
      await treasury.deposit(parseUnits('1000', 'gwei'));
    });
    ...
    it("should change farmer waiting deposit", async () => {
      ...
      expect(farmersDataAfter.waitingAmount.toNumber()).to.be.equal(farmersDataBefore.waitingAmount.add(1000).toNumber());
    });
    ...
    it("should change total waiting amount by same value", async () => {
      ...
      expect(totalWaitingAfter.toNumber()).to.be.equal(totalWaitingBefore.add(1000).toNumber());
    });
    ...
    it("should not change total available amount", async () => {
      ...
      expect(totalWaitingAfter.toNumber()).to.be.equal(totalWaitingBefore.toNumber());
    });
    ...
    it("should increase treasury token balance", async () => {
      ...
      expect(balanceBefore.toNumber()).to.be.equal(balanceAfter.sub(parseUnits('1000', 'gwei')).toNumber());
    });
    ...
    it("should decrease users token balance", async () => {
      ...
      await tx.wait();
      let balanceAfter = await depositedToken.balanceOf(signer.address);
      expect(balanceAfter.toString()).to.be.equal(balanceBefore.sub(parseUnits('1000', 'gwei')).toString());
    });
    ...
    describe("epoch -1", async () => {
      ...
      it("should not sync farmer", async () => {
        ...
        expect(events.filter((x) => x.name === "FarmerSynced").length).to.be.equal(0);
      });
      ...
      it("should emit deposit event", async () => {
        ...
        let receipt = await tx.wait();
        let events = await parseLogs("FarmersTreasury", receipt.events);
        expect(events.filter((x) => x.name === "FundsDeposited").length).to.be.equal(1);
      });
    });
    ...
    describe("epoch > -1", async () => {
      ...
      it("should sync farmer", async () => {
        ...
        let receipt = await tx.wait();
        let events = await parseLogs("FarmersTreasury", receipt.events);
        expect(events.filter((x) => x.name === "FarmerSynced").length).to.be.equal(1);
      });
