...
//  * @param {string} params.mintConfig.contractOwnerAddress - Impersonate address of mintable assetContract contract owner
  ...
  describe.only(`${params.name}`, () => {
    ...
    before(async function () {
      ...
      if (depositAsset === WETH_ADDRESS[chainId]) {
        ...
        await assetContract
          .connect(whaleSigner)
          .transfer(user, parseEther("300"));
      } else {
        await assetContract
          .connect(whaleSigner)
          .transfer(admin, parseUnits("10000000", depositAssetDecimals));
        await assetContract
          .connect(whaleSigner)
          .transfer(user, parseUnits("10000000", depositAssetDecimals));
      }
    });
    ...
    describe("#initialize", () => {
      ...
      it("initializes with correct values", async function () {
        ...
        assert.equal(await vaultContract.owner(), owner);
        assert.equal(await vaultContract.keeper(), keeper);
        assert.equal(await vaultContract.feeRecipient(), feeRecipient);
        assert.equal(
    ...
    describe("#setCap", () => {
      ...
      it("should revert if not owner", async function () {
        await expect(
          vaultContract.connect(userSigner).setCap(parseEther("10"))
      ...
      it("should revert when depositing over the cap", async function () {
        ...
        await vaultContract.connect(ownerSigner).setCap(capAmount);
    ...
    describe("#setNewKeeper", () => {
      ...
      it("set new keeper to owner", async function () {
        assert.equal(await vaultContract.keeper(), keeper);
        await vaultContract.connect(ownerSigner).setNewKeeper(owner);
        assert.equal(await vaultContract.keeper(), owner);
      });

      it("reverts when not owner call", async function () {
        await expect(vaultContract.setNewKeeper(owner)).to.be.revertedWith(
    ...
    describe("#setFeeRecipient", () => {
      ...
      it("reverts when not owner call", async function () {
        await expect(vaultContract.setFeeRecipient(owner)).to.be.revertedWith(
          "caller is not the owner"
        );
      });

      it("changes the fee recipient", async function () {
        await vaultContract.connect(ownerSigner).setFeeRecipient(owner);
        assert.equal(await vaultContract.feeRecipient(), owner);
      });
    });
    ...
    describe("#setManagementFee", () => {
      ...
      it("setManagementFee to 0", async function () {
        await vaultContract.connect(ownerSigner).setManagementFee(0);
    ...
    describe("#setPerformanceFee", () => {
      ...
      it("setPerformanceFee to 0", async function () {
        await vaultContract.connect(ownerSigner).setPerformanceFee(0);
    ...
    describe("#shares", () => {
      ...
      it("shows correct share balance after redemptions", async function () {
        ...
        let userDepositReceipt = await vaultContract.depositReceipts(user);
        let ownerDepositReceipt = await vaultContract.depositReceipts(owner);
        ...
        userDepositReceipt = await vaultContract.depositReceipts(user);
        ownerDepositReceipt = await vaultContract.depositReceipts(owner);
    ...
    if (depositAsset === WETH_ADDRESS[chainId]) {
      describe("#depositETH", () => {
        ...
        it("reverts when no value passed", async function () {
          await expect(
            vaultContract.connect(userSigner).depositETH({ value: 0 })
          ).to.be.revertedWith("15");
        });
        ...
        it("reverts when minimum shares are not minted", async function () {
          await expect(
            vaultContract.connect(userSigner).depositETH({
    ...
    describe("#deposit", () => {
      ...
      //   await vaultContract.connect(ownerSigner).deposit(depositAmount);
      ...
      it("updates the previous deposit receipt", async function () {
        ...
        } = await vaultContract.depositReceipts(user);
    ...
    describe("#depositFor", () => {
      ...
      it("creates a pending deposit", async function () {
        ...
        const { round2, amount2 } = await vaultContract.depositReceipts(user);
      ...
      //   await vaultContract.connect(ownerSigner).depositFor(depositAmount, creditor);
    ...
    describe("#initiateWithdraw", () => {
      ...
      it("creates withdrawal from unredeemed shares", async function () {
        ...
        const { round, shares } = await vaultContract.withdrawals(user);
    ...
    describe("#completeWithdraw", () => {
      ...
      it("reverts when not initiated", async function () {
        await expect(
          vaultContract.connect(ownerSigner).completeWithdraw()
    ...
    //     await mockRegistry.connect(adminSigner).setIsTrue(false);
    ...
    //     let buyerBalanceBefore = await assetContract.balanceOf(user);
    //     let totalBalanceBefore = await vaultContract.totalBalance();
    ...
    //     let buyerBalanceAfter = await assetContract.balanceOf(user);
    //     let totalBalanceAfter = await vaultContract.totalBalance();
